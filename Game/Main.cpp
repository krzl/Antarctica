...
void main()
{
	...
	try
	{
		...
		{
			{
				Ref<Renderer::StaticMesh> staticMesh = Application::Get().GetWorld().Spawn<Renderer::StaticMesh>();
				...
				staticMesh->GetStaticMeshComponent()->SetMesh(
					AssetManager::GetAsset<Mesh>("../Resources/Meshes/Running Arc.fbx"));
