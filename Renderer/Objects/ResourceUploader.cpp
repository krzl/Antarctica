...
namespace Renderer
{
	void ResourceUploader::UploadResource(D3D12_RESOURCE_DESC& resourceDesc,
										  ...
										  ComPtr<ID3D12Resource>& uploadBuffer, ComPtr<ID3D12Resource>& buffer)
	{
		...
		CD3DX12_RESOURCE_DESC uploadBufferInfo = CD3DX12_RESOURCE_DESC::Buffer(uploadBufferSize);
		...
		CD3DX12_RESOURCE_BARRIER transitionToCopyDest = CD3DX12_RESOURCE_BARRIER::Transition(buffer.Get(),
																							 D3D12_RESOURCE_STATE_COMMON,
		...
		CD3DX12_RESOURCE_BARRIER transitionToRead = CD3DX12_RESOURCE_BARRIER::Transition(buffer.Get(),
																						 D3D12_RESOURCE_STATE_COPY_DEST,
																						 targetState);
