...
public:
	...
	void DrawCylinder(Point3D center,
		Quaternion            rotation,
		float                 height,
		float                 width,
		float                 duration = 0.0f,
...
private:
	...
	static std::shared_ptr<ElementBuilder> ReserveLines(uint32_t lineCount, const uint32_t segmentCount);

	void AppendLine(Point3D              start,
		Point3D                          end,
		std::shared_ptr<ElementBuilder>& drawElement,
		uint32_t                         segmentCount = SEGMENTS_PER_LINE,
		float                            width        = LINE_WIDTH) const;
};
