...
void ImGuiManager::Update()
{
	...
	const MousePosition mousePos    = inputManager->GetMousePosition();

	io.MousePos     = ImVec2((float) mousePos.first, (float) mousePos.second);
	io.MouseDown[0] = inputManager->IsLeftMousePressed();
	io.MouseDown[1] = inputManager->IsRightMousePressed();
...
std::vector<Rendering::QueuedRenderObject>& ImGuiManager::Render()
{
	m_wasRendered = true;
	ImGui::Render();
	...
	for (uint32_t drawListId = 0; drawListId < (uint32_t) imDrawData->CmdListsCount; ++drawListId)
	{
		...
		for (const ImDrawCmd& cmd : drawList->CmdBuffer)
		{
			...
			m_renderObjectsCache[cmdIndex].m_perObjectBuffer = Rendering::PerObjectBuffer::DEFAULT_BUFFER;
