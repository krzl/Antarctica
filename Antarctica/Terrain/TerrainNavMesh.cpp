...
Terrain::NavMeshEdge Terrain::GenerateNavMeshEdge(const uint8_t edgeDirection, std::vector<uint8_t>& lookup, const uint32_t x, const uint32_t y,
												  const int32_t deltaX,
												  const int32_t deltaY) const
{
...
Navigation::NavMesh Terrain::CreateNavMesh() const
{
	...
	for (auto [id, usageCount] : vertexUsageCount)
	{
		if (usageCount != 0)
		{
			DebugDrawManager::GetInstance()->DrawSphere(GetPos(id), 0.3f, 1000.0f, Color::red, 5);
			vertices.emplace_back(GetPos(id));
		}
	}

	return Navigation::NavMesh();
}
